...
function fillBackground(ctx, w, h, opts) {
  ...
  if (opts.bgMode === "gradient") {
    ...
    grad.addColorStop(1, opts.bgColor2 || opts.bgColor);
    ctx.fillStyle = grad;
...
export function patternToSVG(opts, rasterDataUrl) {
  ...
  if (transparent) {
    ...
    if (opts.gradientType === "radial") {
      defs = `<radialGradient id="bgGrad" cx="50%" cy="50%" r="75%">
        <stop offset="0%" stop-color="${bgColor}"/>
        <stop offset="100%" stop-color="${opts.bgColor2 || bgColor}"/>
      </radialGradient>`;
    } else {
      const angle = opts.gradientAngle || 0;
      defs = `<linearGradient id="bgGrad" gradientTransform="rotate(${angle} 0.5 0.5)">
        <stop offset="0%" stop-color="${bgColor}"/>
        <stop offset="100%" stop-color="${opts.bgColor2 || bgColor}"/>
      </linearGradient>`;
    }
    bg = `<rect width="${w}" height="${h}" fill="url(#bgGrad)"/>`;
  } else {
    bg = `<rect width="${w}" height="${h}" fill="${bgColor}"/>`;
  }
